...
function decodeGBK(str) {
  ...
  const charMap = {}; // 存储 str 中间出现的特殊字符 0-9, a-z, A-Z, -._
  let result = [];
  ...
  arr.forEach((currentValue, index) => {
    if (currentValue.length > 2) {
      charMap[index] = currentValue.substring(2);
      arr[index] = currentValue.substring(0, 2);
    }
    arr[index] = '0x' + arr[index].toLowerCase();
  });

  result = iconv.decode(Buffer.from(arr), 'GBK').split('');
